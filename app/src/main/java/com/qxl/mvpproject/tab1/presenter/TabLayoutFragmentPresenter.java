 ...
 */
public class TabLayoutFragmentPresenter extends BasePresenter<TabLayoutFragment,BaseModel> {
    RvAdapter adapter;

    public TabLayoutFragmentPresenter(Context context) {
        super(context);
    }
    ...
    public RvAdapter getAdapter() {
        if (adapter == null) {
            adapter = new RvAdapter(getContext(), getList(), R.layout.item_tablayout_recyclerview);
        }
